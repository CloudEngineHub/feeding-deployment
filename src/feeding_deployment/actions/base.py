...
class HighLevelAction(abc.ABC):
    ...
    def process_behavior_tree_node_addition(
        ...
        bt = load_behavior_tree(bt_filepath, self)
        # Get the anchor node.
        node = bt.get_node(anchor_node_name)
        ...
        if not isinstance(node.parent, SequenceBehaviorTreeNode):
            # For now we'll assume that this case only happens when the node is
            # the root itself.
            assert node.parent is None
            assert node is bt
            # Wrap it in a sequence node.
            bt = SequenceBehaviorTreeNode("Autogen", "Autogenerated sequence node", [node])
        # Now the node is part of a sequence.
        assert isinstance(node.parent, SequenceBehaviorTreeNode)
